...
select s.student_name,e.course_name from students s left join enrollments e on s.student_id=e.student_id
union all
select s.student_name,e.course_name from students s right join enrollments e on s.student_id=e.student_id where s.student_id is null;
...
-- 9
select s.student_name,sum(e.fee) as total_fee from students s left join enrollments e on s.student_id=e.student_id 
group by s.student_name ;
